...
if os.path.isfile(os.path.join(exp_dir, 'explanations.pkl')):
    ...
    for y_class in range(y_test.shape[1]):
        ...
        for current_input in tqdm(X_test):
            ...
            for j in range(0, num_samples, batch_size):
                ...
                #Generate Noise
                batch_noise = np.random.normal(0, noise, batch_input.shape).astype(np.float32)

                # Generate Noisey Inputs
                batch_noisey_input = batch_input + batch_noise

                with tf.GradientTape() as tape:
                    tape.watch(batch_noisey_input)

                    batch_predictions = model(batch_noisey_input)
                    ### Get prediction of the predicted class
                    batch_predictions = batch_predictions[:, y_class]

                batch_gradients = tape.gradient(batch_predictions, batch_noisey_input)

                attribution_array.append(batch_gradients)
    ...
    ## Clean
    gc.collect()
    del (attribution_array, batch_gradients, 
         batch_predictions, reps, batch_input, exp, batch_noise, batch_noisey_input)
    gc.collect()
