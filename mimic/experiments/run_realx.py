...
### Use Predictions as Labels

def use_predictions(x, y):
    yp = model(x)
    return x, yp
train_data = train_data.map(use_predictions)
val_data = val_data.map(use_predictions)
test_data = test_data.map(use_predictions)
